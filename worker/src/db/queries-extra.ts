...
// ─── Active sessions (distinct client+account in last N min) ─────────────────
...
export async function fetchActiveSessions(env: Env, withinMs = 5 * 60 * 1000): Promise<ActiveSession[]> {
	...
	const res = await env.DB.prepare(
		`SELECT client, account_email, COUNT(*) as calls, MAX(ts) as lastSeen,
		        (SELECT model FROM api_logs a2 WHERE a2.client = a1.client AND a2.account_email = a1.account_email AND a2.ts >= ? ORDER BY ts DESC LIMIT 1) as lastModel
		 FROM api_logs a1
		 WHERE ts >= ?
		 GROUP BY client, account_email
